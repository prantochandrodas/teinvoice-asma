            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"> Product Tag List </h3>
                        @if (auth_admin_user_permission('productTag.create'))
                            <a href="{{ route('admin.productTag.create') }}" class="btn btn-success float-right">
                                <i class="fa fa-pencil-alt"></i> Add New Product Tag
                            </a>
                        @endif
                    </div>
                    <div class="card-body">
  ...
  <script>
    window.onload = function(){
        ...
        var table = $('#yajraDatatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('admin.productTag.getProductTags') !!}',
            columns: [
        ...
        $('#yajraDatatable').on('click', '.updateStatus', function(){
            ...
            var product_tag_id  = status_object.attr('product_tag_id');
            var status          = status_object.attr('status');
            var url             = "{{ route('admin.productTag.updateStatus') }}";
        ...
        $('#yajraDatatable').on('click', '.delete-btn', function(){
            var status_object   = $(this);
            var product_tag_id  = status_object.attr('product_tag_id');
            var url             = "{{ route('admin.productTag.delete') }}";
