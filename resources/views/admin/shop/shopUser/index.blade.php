      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Shop User</li>
          </ol>
        </div>
      </div>
    </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"> Shop Users List </h3>
                        @if (auth_admin_user_permission('shopUser.create'))
                            <a href="{{ route('admin.shopUser.create') }}" class="btn btn-success float-right">
                                <i class="fa fa-pencil-alt"></i> Add Shop User
                            </a>
                        @endif
                    </div>
                    <div class="card-body">
                        <table id="yajraDatatable" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th width="10%" class="text-center"> Shop </th>
  ...
  <script>
    window.onload = function(){
        ...
        var table = $('#yajraDatatable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('admin.shopUser.getShopUsers') !!}',
            columns: [
                ...
                { data: 'shop.name', name: 'shop.name', class : "text-center" },
        ...
        $('#yajraDatatable').on('click', '.view-modal', function(){
            var shop_user_id = $(this).attr('shop_user_id');
            var url         = "{{ route('admin.shopUser.show', ":shop_user_id") }}";
            url = url.replace(':shop_user_id', shop_user_id);
            $('#showResult').html('');
            if(shop_user_id.length != 0){
            $.ajax({
                cache   : false,
                type    : "GET",
        ...
        $('#yajraDatatable').on('click', '.updateStatus', function(){
            ...
            var shop_user_id  = status_object.attr('shop_user_id');
            var status    = status_object.attr('status');
            var url       = "{{ route('admin.shopUser.updateStatus') }}";
        ...
        $('#yajraDatatable').on('click', '.delete-btn', function(){
            var status_object = $(this);
            var shop_user_id  = status_object.attr('shop_user_id');
            var url       = "{{ route('admin.shopUser.delete') }}";
