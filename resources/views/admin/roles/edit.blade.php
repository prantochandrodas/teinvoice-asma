...
@section('content')
    ...
    <div class="main-card mb-3 card">
        <div class="card-body">
            ...
            <form id="signupForm" class="col-md-10 mx-auto" method="post" action="{{ route('admin.role.update', $role->id) }}" novalidate="novalidate" onsubmit="return confirmSubmit()">
                @csrf
                <input type="hidden" name="_method" value="PUT">
                <div class="form-group">
                    <label for="name">Name</label>
                    <div>
                        <input type="text" class="form-control" id="name" name="name" pattern="[A-Za-z]+" value="{{ $role->name }}" placeholder="Role Name">
                    </div>
                </div>
                        ...
                        <select id="guard_name" name="guard_name" class="form-control" data-placeholder="Select Role Type">
                            ...
                            if(count($guards) > 0) {
                                foreach($guards as $k=>$v) {
                                    $selected = ($role->guard_name == $v) ? "selected" : "";
                    ...
                    <div style="border: 1px solid #ccc; border-radius: 4px; min-height: 80px; padding: 10px">
                        ...
                        <div id="permission_data" class="position-relative form-check form-check-inline">
                            ...
                            @foreach($permissions as $permission)
                                @php
                                    $i++;
                                    $checked = (in_array($permission->id, $role_permissions)) ? "checked" : "";
                                @endphp
                                <label class="form-check-label">
                                    <input type="checkbox" name="permissions[]" class="form-check-input permission_item" value="{{ $permission->id }}" {{ $checked }}> <b>{{ $permission->name }}</b>
                                </label>&nbsp;
                            @endforeach
